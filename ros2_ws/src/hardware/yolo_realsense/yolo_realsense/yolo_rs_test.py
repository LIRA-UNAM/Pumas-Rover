...
class YoloStoneDetector(Node):
    def __init__(self):
        ...
        self.error_pub = self.create_publisher(Float32, '/yolo/pixel_error_x', 10)

        # Suscripciones
        self.camera_info_sub = self.create_subscription(CameraInfo, '/camera/camera/color/camera_info', self.info_cb, 10)
        ...
        self.get_logger().info('Visión Pumas-Rover: Publicando Error de Centrado activo.')
    ...
    def synced_callback(self, rgb_msg, depth_msg):
        ...
        for result in results:
            for box in result.boxes:
                ...
                # Calcular Z 
                Z_current = np.median(valid_depths) / 1000.0 if depth_frame.dtype == np.uint16 else np.median(valid_depths)
        ...
        if closest_point:
            ...
            pixel_error = float(self.cx - best_u)
            
            err_msg = Float32()
            err_msg.data = pixel_error
            self.error_pub.publish(err_msg)

            
            self.get_logger().info(f'[TARGET] Z={Z:.2f}m | Error Pixel={pixel_error:.1f}')
            ...
            cv2.line(annotated, (int(self.cx), 0), (int(self.cx), frame.shape[0]), (255, 0, 0), 2)
            cv2.putText(annotated, f"Err: {pixel_error:.1f}px", (50, 50), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
            
            cv2.imshow('YOLO ERROR', annotated)
            cv2.waitKey(1)
